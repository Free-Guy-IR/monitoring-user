...
هدف‌ها:
- هیـچ لاگی حذف نمی‌شود (sources: connections_log.txt).
- جلوی کندی/کرش با زیاد شدن لاگ‌ها گرفته می‌شود:
  * سمت سرور یک شاخص سبک برای هر کاربر (base) نگهداری می‌شود، نه همه‌ی رخدادها.
  * سمت کلاینت دیگر همه‌ی رخدادها را در حافظه نگه نمی‌دارد؛ فقط snapshot تجمیع‌شده را می‌گیرد.
  * تاریخچه‌ی هر کاربر به‌صورت صفحه‌به‌صفحه (paging) مستقیماً از فایل لاگ خوانده می‌شود.
- رتبه‌بندی (⚠️/⛔) برقرار است و بر مبنای نام کاربرِ base عمل می‌کند (حذف پیشوند عددی قبل از نقطه).
...
const PAGE_SIZE = 3000; // یا 5000، مطابق سقف سرور
...
function renderModal(){
  ...
  modalTitle.innerText = `گزارش — ${modalBase} (نمایش ${rows.length} از ${modalAll.length})`;
  const frag = document.createDocumentFragment();
  for(const r of rows){
